...
const TABS = ["Dashboard","Traders","Upload Data","Rules","Branding","Embed Widget","Preview AI"];export default function AdminPanel() {
  ...
  const statusColor = (s:string) => s==="Active"?"#10b981":s==="Passed"?"#3b82f6":"#ef4444";function saveRules() {
    setSavedRules({...rules});
    setRulesSaved(true);
  ...
  function parseCSV(text: string) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      parsed.push({
        id: obj.id||`PF-${String(i).padStart(3,"0")}`,
        name: obj.name||"Unknown", phase: obj.phase||"Phase 1",
        balance: parseFloat(obj.balance)||0,
        dd: parseFloat(obj.drawdown||obj.dd||"0"),
        dailyLoss: parseFloat(obj["daily loss"]||obj.dailyloss||"0"),
    ...
    setTraders(parsed); setCsvData(parsed);
    setCsvSuccess(`${parsed.length} traders import ho gaye!`);
  }
  ...
  async function sendChat() {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          system: `You are ${brandName} PropFirm AI. Rules: Daily Loss ${savedRules.dailyLossLimit}%, Drawdown ${savedRules.maxDrawdown}%, Phase1 ${savedRules.phase1Target}%, Phase2 ${savedRules.phase2Target}%, Payout ${savedRules.payoutSplit}/${100-savedRules.payoutSplit}. Traders: ${traders.slice(0,5).map(t=>`${t.name}(${t.id}):${t.phase},$${t.balance},DD${t.dd}%,${t.status}`).join("; ")}. Detect language, reply same language, under 80 words, end with 2-3 followup bullets.`,
          messages:[{role:"user",content:userMsg}]
        })
      });
      const data = await res.json();
      setChatMessages(prev => [...prev, {role:"ai",text:data.reply||"Error. Dobara try karein."}]);
  ...
  async function sendChat() {
    ...
    try {
      ...
      const data = await res.json();
      setChatMessages(prev => [...prev, {role:"ai",text:data.reply||"Error aaya, dobara try karein."}]);
    } catch { setChatMessages(prev => [...prev, {role:"ai",text:"Connection error."}]); }
    setAiLoading(false);
  }const embedCode = `<script src="https://propfirm-ai.vercel.app/widget.js" data-firm="${brandName}" data-color="${brandColor}"></script>`;
      ...
      <div style={{width:220,background:"#fff",borderRight:"1px solid #e5e7eb",padding:"20px 12px"}}>
        ...
        {TABS.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{
            ...
            background:activeTab===tab?brandColor+"18":"transparent",
            color:activeTab===tab?brandColor:"#6b7280",
            fontWeight:activeTab===tab?600:400,fontSize:13,fontFamily:"inherit"
          }}>{tab}</button>
        ))}
      </div>
      <div style={{flex:1,padding:32,overflowY:"auto"}}>{activeTab==="Dashboard" && (
          <div>
            ...
            <div style={{background:"#fff",borderRadius:12,border:"1px solid #e5e7eb",overflow:"hidden"}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                <thead><tr style={{background:"#f9fafb"}}>
                  {["ID","Name","Phase","Balance","Drawdown","Status"].map(h => <th key={h} style={{padding:"10px 16px",textAlign:"left",color:"#6b7280",fontWeight:500}}>{h}</th>)}
                </tr></thead>
          ...
          <div>
            ...
            <div style={{background:"#fff",borderRadius:12,border:"1px solid #e5e7eb",overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                <thead><tr style={{background:"#f9fafb"}}>
                  {["ID","Name","Phase","Balance","DD%","Daily Loss","Profit%","Violations","Status"].map(h => <th key={h} style={{padding:"12px 14px",textAlign:"left",color:"#6b7280",fontWeight:500}}>{h}</th>)}
                </tr></thead>
                <tbody>{traders.map((t,i) => (
                  <tr key={t.id} style={{borderTop:"1px solid #f3f4f6",background:i%2===0?"#fff":"#fafafa"}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
              {[
                {label:"Daily Loss Limit (%)",key:"dailyLossLimit",min:1,max:20},
                {label:"Max Drawdown (%)",key:"maxDrawdown",min:1,max:30},
                {label:"Phase 1 Profit Target (%)",key:"phase1Target",min:1,max:20},
                {label:"Phase 2 Profit Target (%)",key:"phase2Target",min:1,max:20},
                {label:"Min Trading Days",key:"minTradingDays",min:1,max:30},
                {label:"Payout Split (Trader %)",key:"payoutSplit",min:50,max:95},
                {label:"Payout Processing (days)",key:"payoutProcessing",min:1,max:14},
                {label:"News Window (minutes)",key:"newsWindow",min:1,max:30},
              ].map(f => (
                <div key={f.key} style={{background:"#fff",borderRadius:10,padding:18,border:"1px solid #e5e7eb"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <input type="range" min={f.min} max={f.max} value={(rules as any)[f.key]}
                      onChange={e=>setRules(prev=>({...prev,[f.key]:Number(e.target.value)}))}
                      style={{flex:1,accentColor:brandColor}}/>
                    <input type="number" min={f.min} max={f.max} value={(rules as any)[f.key]}
                      onChange={e=>setRules(prev=>({...prev,[f.key]:Number(e.target.value)}))}
                      style={{width:60,padding:"6px 8px",borderRadius:6,border:"1px solid #d1d5db",textAlign:"center",fontSize:14,fontWeight:600,color:brandColor,fontFamily:"inherit"}}/>
                  </div>
                </div>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24}}>
              {[{label:"Weekend Holding",key:"weekendHolding",desc:"Traders positions weekend hold kar sakte hain"},{label:"News Trading",key:"newsTrading",desc:"High-impact news ke time trading allow hai"}].map(f=>(
                <div key={f.key} style={{background:"#fff",borderRadius:10,padding:18,border:"1px solid #e5e7eb",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:500,color:"#374151"}}>{f.label}</div>
                    <div style={{fontSize:12,color:"#9ca3af",marginTop:4}}>{f.desc}</div>
                  </div>
                  <div onClick={()=>setRules(prev=>({...prev,[f.key]:!(prev as any)[f.key]}))}
                    style={{width:44,height:24,borderRadius:12,cursor:"pointer",background:(rules as any)[f.key]?brandColor:"#d1d5db",position:"relative",flexShrink:0,transition:"background 0.2s"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
              <div style={{background:"#fff",borderRadius:12,padding:24,border:"1px solid #e5e7eb"}}>
                ...
                <input value={brandName} onChange={e=>setBrandName(e.target.value)}
                  style={{width:"100%",padding:"10px 12px",borderRadius:8,border:"1px solid #d1d5db",fontSize:14,marginBottom:16,boxSizing:"border-box",fontFamily:"inherit"}}/>
                <label style={{fontSize:13,color:"#6b7280",display:"block",marginBottom:6}}>Brand Color</label>
                <div style={{display:"flex",gap:12,alignItems:"center"}}>
                  <input type="color" value={brandColor} onChange={e=>setBrandColor(e.target.value)}
                    style={{width:48,height:40,borderRadius:6,border:"1px solid #d1d5db",cursor:"pointer",padding:2}}/>
                  <input value={brandColor} onChange={e=>setBrandColor(e.target.value)}
                    style={{flex:1,padding:"10px 12px",borderRadius:8,border:"1px solid #d1d5db",fontSize:13,fontFamily:"monospace"}}/>
                </div>
              </div>
          ...
          <div>
            ...
            <p style={{color:"#6b7280",fontSize:14,marginBottom:24}}>Yeh code apni website mein paste karein — AI chat bot appear ho jayega</p>
            ...
            <button onClick={()=>navigator.clipboard.writeText(embedCode)}
              style={{background:brandColor,color:"#fff",border:"none",borderRadius:8,padding:"10px 20px",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              Copy Code
            </button>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{background:"#fff",borderRadius:12,border:"1px solid #e5e7eb",display:"flex",flexDirection:"column",height:480}}>
              ...
              <div style={{padding:12,borderTop:"1px solid #e5e7eb",display:"flex",gap:8}}>
                <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!aiLoading&&sendChat()}
                  placeholder="Koi bhi sawal likhein (Urdu/English)..."
                  style={{flex:1,padding:"10px 14px",borderRadius:8,border:"1px solid #d1d5db",fontSize:13,outline:"none",fontFamily:"inherit"}}/>
                <button onClick={sendChat} disabled={aiLoading||!chatInput.trim()}
                  style={{background:brandColor,color:"#fff",border:"none",borderRadius:8,padding:"0 18px",fontSize:13,fontWeight:600,cursor:"pointer",opacity:aiLoading?0.6:1}}>Send</button>
              </div>
            </div>
          </div>
        )}
